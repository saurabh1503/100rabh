...
class ProductLinks
{
    ...
    )
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getCustomtypeReadonly()) {
		 ...
		 if(isset($links[self::SIZE_NAME]) && !$product->getSizetypeReadonly()) {

            $links = (isset($links[self::SIZE_NAME])) ? $links[self::SIZE_NAME] : $product->getSizetypeLinkData();
            if (!is_array($links)) {
                $links = [];
            }

            if ($product->getSizetypeLinkData()) {
                $links = array_merge($links, $product->getSizetypeLinkData());
            }
            $newLinks = [];
            $existingLinks = $product->getProductLinks();
            foreach ($links as $linkRaw) {
            ...
            $existingLinks = $this->removeSizeUnExistingLinks($existingLinks, $newLinks);
            $product->setProductLinks(array_merge($existingLinks, $newLinks));
        }
     ...
     * @return array
     */
    private function removeSizeUnExistingLinks($existingLinks, $newLinks)
    {
        $result = [];
        foreach ($existingLinks as $key => $link) {
            $result[$key] = $link;
            if ($link->getLinkType() == self::SIZE_NAME) {
                $exists = false;
                foreach ($newLinks as $newLink) {
                    if ($link->getLinkedProductSku() == $newLink->getLinkedProductSku()) {
                        $exists = true;
