...
class Product extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    ...
    {
        $imagesArr = [];
        $imagestr = $this->getData('image_id');
        if($imagestr && is_string($imagestr)) {
            $imagesArr = explode(',',$imagestr);
        }
        return $imagesArr;
    }
    ...
    public function getItemWidth()
    {
        ...
        if ($productPerRow) {
            if (!$this->getShowCarousel()) {
                $productItemWidth = 'style="max-width: ' . 100 / $productPerRow . '%; width: ' . 100 / $productPerRow . '%; flex-basis:' . 100 / $productPerRow . '%"';
            }
        }
    ...
    protected function _beforeToHtml()
    {
	 ...
	 /**
     *
     *  Function used call featured products in custom order
     *
     * @param $product_id
     * @return \Magento\Catalog\Model\Product
     */
	public function CustomFeaturedProduct($product_id){
